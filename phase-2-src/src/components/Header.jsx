...
function Header({ currentStep, completedSteps, onMaximizeClick, onNavigate }) {
    const steps = [1, 2, 3, 4]
        ...
        <Fragment>
            <header className="header">
                ...
                <div className="steps">
                    {steps.map((num) => (
                        <Fragment key={num}>
                            <button onClick={() => { onNavigate(num) }} className={`step ${currentStep === num ? 'current' : ''} ${completedSteps >= num ? 'done' : ''}`} disabled={completedSteps + 1 < num}>{num}</button>
